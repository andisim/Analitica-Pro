...
export const getProductCategory = (productName: string): string => {
  ...
  if (name.includes("denim") || name.includes("jeans") || name.includes("overalls")) return "Denim & Overalls";
  if (name.includes("tailored") || name.includes("pleated") || name.includes("wool") || name.includes("trousers")) {
    if (name.includes("linen") || name.includes("corduroy") || name.includes("seersucker")) {
      return "Casual Trousers";
    }
    return "Sastrería & Formal";
  }
  if (name.includes("chinos") || name.includes("khaki")) return "Chinos & Khakis";
  if (name.includes("joggers") || name.includes("performance")) return "Deportivo & Athleisure";
  if (name.includes("canvas") || name.includes("work")) return "Trabajo & Workwear";
  if (name.includes("shorts") || name.includes("cargo")) return "Shorts & Cargos";
  return "Otros Pantalones";
};

export const getPaymentMethodTranslations = (method: string): string => {
  switch (method) {
    case "Credit Card": return "Tarjeta de Crédito";
    case "Debit Card": return "Tarjeta de Débito";
    case "eWallet": return "Billetera Digital (eWallet)";
